...
def apply_filters(companies: list[dict], filters: dict) -> list[dict]:
    ...
    min_year = filters.get("min_year_founded")
    max_year = filters.get("max_year_founded")
    ...
    for co in companies:
        ...
        # ── Year founded ────────────────────────────────────────────────────
        yr = co.get("year_founded")
        if min_year is not None:
            passed = yr is not None and int(yr) >= min_year
            match_log["min_year_founded"] = passed
            if not passed:
                keep = False
        if max_year is not None:
            passed = yr is not None and int(yr) <= max_year
            match_log["max_year_founded"] = passed
            if not passed:
                keep = False
